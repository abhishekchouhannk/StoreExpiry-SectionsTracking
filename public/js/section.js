...
async function loadCleaning() {
  ...
  wrap.innerHTML = '<div class="col-12 text-center py-4 text-muted small">Loading…</div>';
  try {
  ...
  } catch (e) { wrap.innerHTML = `<div class="col-12 text-danger small">${e.message}</div>`; }
}
...
function renderWeekCards(container, data, type) {
  ...
  for (let w = 1; w <= 4; w++) {
    ...
    const colors = { done: 'success', missed: 'danger', pending: 'secondary' };
    const labels = { done: type === 'cleaning' ? 'Cleaned' : 'Checked', missed: 'Missed', pending: 'Pending' };
    const c = colors[status], label = labels[status];

    let body = '';
    if (entry && type === 'cleaning') {
      body = `
        <div class="row small text-muted mb-1"><div class="col-5 fw-semibold">Date</div><div class="col-7">${fmtDate(entry.dateCleaned)}</div></div>
        <div class="row small text-muted mb-1"><div class="col-5 fw-semibold">Cleaned By</div><div class="col-7">${entry.cleanedBy || '—'}</div></div>
        <div class="row small text-muted"><div class="col-5 fw-semibold">Comments</div><div class="col-7">${entry.comments || '—'}</div></div>`;
    } else if (entry && type === 'planogram') {
      body = `
        <div class="row small text-muted mb-1"><div class="col-5 fw-semibold">Date</div><div class="col-7">${fmtDate(entry.dateChecked)}</div></div>
        <div class="row small text-muted mb-1"><div class="col-5 fw-semibold">Checked By</div><div class="col-7">${entry.checkedBy || '—'}</div></div>
        <div class="row small text-muted mb-1"><div class="col-5 fw-semibold">Comments</div><div class="col-7">${entry.comments || '—'}</div></div>
        <div class="row small text-muted"><div class="col-5 fw-semibold">Plano Fixed</div><div class="col-7">${entry.planogramFixed ? '<i class="bi bi-check-circle-fill text-success"></i> Yes' : '<i class="bi bi-x-circle text-secondary"></i> No'}</div></div>`;
    } else {
      body = '<span class="text-muted small fst-italic">No entry yet.</span>';
    }

    const deleteBtn = entry
      ? `<button class="btn btn-sm del-entry" title="Delete" onclick="deleteWeekEntry('${type}','${entry._id}')"><i class="bi bi-trash3 text-muted"></i></button>`
      : '';
      ...
      <div class="col-12 col-md-6">
        <div class="card log-card">
          <div class="card-header d-flex align-items-center gap-2 py-2">
            <span class="week-badge bg-${c} ${c==='warning'?'text-dark':'text-white'}">${w}</span>
            Week ${w}
            <span class="ms-auto badge bg-${c} bg-opacity-15 text-${c}">${label}</span>
            ${deleteBtn}
          </div>
...
function updateWeekDropdown(selectId, data) {
  ...
  if (sel.options.length === 0) {
    sel.innerHTML = '<option disabled>All weeks logged</option>';
  }
}
...
async function loadExpiry() {
  ...
  try {
    ...
    if (data.length === 0) {
      wrap.innerHTML = '<div class="card log-card"><div class="card-body text-center text-muted py-4">No expiry items found.</div></div>';
      return;
    }
    let rows = '';
    data.forEach((item) => {
      const removedBadge = item.removed
        ? '<span class="badge bg-success bg-opacity-15 text-success"><i class="bi bi-check-circle me-1"></i>Yes</span>'
        : `<button class="btn btn-outline-success btn-sm py-0 px-2" style="font-size:.75rem" onclick="markExpRemoved('${item._id}')"><i class="bi bi-check me-1"></i>Mark Removed</button>`;
      rows += `
        <tr>
          <td class="ps-3">${fmtDate(item.date)}</td>
          <td>${item.item}</td>
          <td class="${item.removed ? '' : 'text-danger fw-semibold'}">${fmtDate(item.expiryDate)}</td>
          <td>${removedBadge}</td>
          <td>${item.signOffBy || '—'}</td>
          <td><button class="btn btn-sm del-entry" onclick="deleteExpiry('${item._id}')"><i class="bi bi-trash3 text-muted"></i></button></td>
        </tr>`;
    });
    wrap.innerHTML = `
      <div class="card log-card"><div class="card-body p-0">
        <table class="table table-hover mb-0">
          <thead class="table-light"><tr>
            <th class="ps-3">Date Found</th><th>Item</th><th>Expiry</th><th>Removed</th><th>Sign Off</th><th></th>
          </tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </div></div>`;
  } catch (e) { wrap.innerHTML = `<div class="text-danger small">${e.message}</div>`; }
}
...
async function loadOrders() {
  ...
  try {
    ...
    if (data.length === 0) {
      wrap.innerHTML = '<div class="card log-card"><div class="card-body text-center text-muted py-4">No order items found.</div></div>';
      return;
    }
    let rows = '';
    data.forEach((item) => {
      const statusBadge = item.ordered
        ? '<span class="badge bg-success bg-opacity-15 text-success">Ordered</span>'
        : `<button class="btn btn-outline-success btn-sm py-0 px-2" style="font-size:.75rem" onclick="markOrdered('${item._id}')"><i class="bi bi-check me-1"></i>Mark Ordered</button>`;
      rows += `
        <tr>
          <td class="ps-3">${fmtDate(item.date)}</td>
          <td>${item.item}</td>
          ...
          <td>${statusBadge}</td>
          <td><button class="btn btn-sm del-entry" onclick="deleteOrder('${item._id}')"><i class="bi bi-trash3 text-muted"></i></button></td>
        </tr>`;
    });
    wrap.innerHTML = `
      <div class="card log-card"><div class="card-body p-0">
        <table class="table table-hover mb-0">
          <thead class="table-light"><tr>
            <th class="ps-3">Date</th><th>Item</th><th>Comments</th><th>Status</th><th></th>
          </tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </div></div>`;
...
function wireFormHandlers() {
  ...
  document.getElementById('f-clean-save').addEventListener('click', async () => {
    ...
    try {
      await api('/api/cleaning-logs', { method: 'POST', body: {
  ...
  document.getElementById('f-plano-save').addEventListener('click', async () => {
    ...
    if (!week || !date) return alert('Week and date are required.');
    try {
      await api('/api/planogram-checks', { method: 'POST', body: {
  ...
  document.getElementById('f-exp-save').addEventListener('click', async () => {
    ...
    try {
      await api('/api/expiry-logs', { method: 'POST', body: {
  ...
  document.getElementById('f-ord-save').addEventListener('click', async () => {
    ...
    if (!item) return alert('Item name is required.');
    try {
      await api('/api/order-items', { method: 'POST', body: {
