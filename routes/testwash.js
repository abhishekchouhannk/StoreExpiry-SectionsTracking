...
router.get('/', async (req, res) => {
  try {
    ...
    const month = req.query.month !== undefined
      ? parseInt(req.query.month, 10) : now.getMonth();
    const year = req.query.year !== undefined
      ? parseInt(req.query.year, 10) : now.getFullYear();
...
router.get('/months', async (req, res) => {
  try {
    ...
    const formatted = months.map(m => ({
      year: m._id.year, month: m._id.month - 1, count: m.count,
    }));
...
router.post('/', async (req, res) => {
  try {
    ...
    const doc = {
      carwashCode, issuedTo, issuedBy, issuedAt,
