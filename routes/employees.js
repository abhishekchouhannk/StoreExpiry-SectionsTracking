...
const {
  isValidEmployeeName, normalizeName, buildAutoAliases,
} = require('../helpers');
router.get('/', async (req, res) => {
  try {
    const { siteId } = req.query;
    if (!siteId) return res.status(400).json({ error: 'siteId required' });
    const list = await req.db.collection('employees')
      .find({ siteId }).sort({ canonicalName: 1 }).toArray();
    res.json(list);
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!siteId || !canonicalName)
      return res.status(400).json({ error: 'siteId and canonicalName required' });
    if (!isValidEmployeeName(canonicalName))
      return res.status(400).json({ error: 'Please enter a valid employee name' });
    ...
    const doc = {
      siteId, canonicalName: canonicalName.trim(),
      aliases: finalAliases, createdAt: new Date(),
    };
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const existing = await req.db.collection('employees')
...
router.delete('/:id', async (req, res) => {
  try {
    await req.db.collection('employees')
