...
router.get('/', async (req, res) => {
  try {
    ...
    res.json(await req.db.collection('sandwich_logs')
...
router.post('/', async (req, res) => {
  try {
    ...
    const exists = await req.db.collection('sandwich_logs')
      .findOne({ siteId, weekStart, sandwich });
    if (exists)
      return res.status(400).json({ error: 'Entry already exists for this sandwich this week' });
    // Flexible schema (day-of-week tallies etc.) — strip _id to prevent injection
...
router.delete('/:id', async (req, res) => {
  try {
    await req.db.collection('sandwich_logs')
