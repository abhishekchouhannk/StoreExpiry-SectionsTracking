...
router.get('/', async (req, res) => {
  try {
    ...
    const docs = await req.db.collection('sections')
...
router.get('/:id', async (req, res) => {
  try {
    const doc = await req.db.collection('sections')
...
router.post('/', async (req, res) => {
  try {
    ...
    const doc  = {
      name, slug, icon: icon || '📦', location: location || '',
      displayOrder: n + 1, siteId, createdAt: new Date(),
    };
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    await Promise.all(
      ['cleaning_logs', 'planogram_checks', 'expiry_logs', 'order_items']
